...
| "No available server" (Traefik) aunque los logs digan `Listening at ...` | El proxy no enruta al puerto del contenedor | (1) **Quita los Port Mappings** (p.ej. `3045:3045`): al usar dominio, el mapping puede desactivar el routing por proxy. (2) **Ports Exposes** = el puerto donde escucha la app (`PORT`, p.ej. `3045`). (3) Asegura el dominio en *Domains*. (4) Reinicia el proxy (Server → Proxy → Restart) y redesplega. Verifica dentro del contenedor: `curl http://127.0.0.1:3045/health`. |
| Healthcheck falla con `curl: not found` / `wget: not found` y Coolify revierte al contenedor viejo | La imagen no traía curl para el healthcheck de Coolify | Ya resuelto: el `Dockerfile` instala `curl`. (Alternativa: desactivar el Healthcheck en la UI de Coolify.) |
